...
async function validate() {
  const parts = process.argv[2].split(/\\s+/);
  const cmd = parts[0];
...
`;
  ...
  return new Promise((resolve) => {
    ...
    dockerArgs.push(
      ...
      "bash",
      "-c",
      `npm install -g ${installCommand.replace(/^npx -y /, "")} > /dev/null 2>&1 && node -e '${validationScript.replace(/'/g, "'\\''")}' '${installCommand}'`
    );
...
`;
  ...
  return new Promise((resolve) => {
    ...
    dockerArgs.push(
      ...
      "bash",
      "-c",
      `pip install uv > /dev/null 2>&1 && uv tool install ${packageName} > /dev/null 2>&1 && python3 -c '${pythonScript.replace(/'/g, "'\\''")}' '${installCommand}'`
    );
